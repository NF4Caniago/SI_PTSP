<?php

class home_controler extends CI_Controller{
    
    public function __construct() {
        parent::__construct();
        //$this->load->model('admin_model');
    }
    
    public function index(){
        $this->load->view('HomeUser_view');
    }
    
    public function detail1() {
        $this->load->view('detail/detail1');
    }
    
    public function detail2() {
        $this->load->view('detail/detail2');
    }
    
    public function detail3() {
        $this->load->view('detail/detail3');
    }
    
    public function detail4() {
        $this->load->view('detail/detail4');
    }
    
    public function detail4_1() {
        $this->load->view('detail/detail4_1');
    }
    
    public function detail4_2() {
        $this->load->view('detail/detail4_2');
    }
    
    public function detail5() {
        $this->load->view('detail/detail5');
    }
    
    public function detail6() {
        $this->load->view('detail/detail6');
    }
    
    public function detail7() {
        $this->load->view('detail/detail7');
    }
    
    public function detail8() {
        $this->load->view('detail/detail8');
    }
    
    public function detail9() {
        $this->load->view('detail/detail9');
    }
    
    public function detail10() {
        $this->load->view('detail/detail10');
    }
    
    public function detail11() {
        $this->load->view('detail/detail11');
    }
    
    public function detail12() {
        $this->load->view('detail/detail12');
    }
    
    public function detail13() {
        $this->load->view('detail/detail13');
    }
    
    public function detail14() {
        $this->load->view('detail/detail14');
    }
    
    public function detail15() {
        $this->load->view('detail/detail15');
    }
    
    public function detail16() {
        $this->load->view('detail/detail16');
    }
    
    public function detail17() {
        $this->load->view('detail/detail17');
    }
    
    public function detail18() {
        $this->load->view('detail/detail18');
    }
    
    public function detail19() {
        $this->load->view('detail/detail19');
    }
    
    public function detail20() {
        $this->load->view('detail/detail20');
    }
    
    public function detail21() {
        $this->load->view('detail/detail21');
    }
    
    public function detail21_1(){
        $this->load->view('detail/detail21_1');
    }

    public function detail21_2() {
        $this->load->view('detail/detail21_2');
    }
    
    public function detail21_3() {
        $this->load->view('detail/detail21_3');
    }
    
    public function detail22() {
        $this->load->view('detail/detail22');
    }
    
    public function detail22_1() {
        $this->load->view('detail/detail22_1');
    }
    
    public function detail22_2() {
        $this->load->view('detail/detail22_2');
    }
    
    public function detail22_3() {
        $this->load->view('detail/detail22_3');
    }
    
    public function detail23() {
        $this->load->view('detail/detail23');
    }
    
    public function detail23_1() {
        $this->load->view('detail/detail23_1');
    }
    
    public function detail23_2() {
        $this->load->view('detail/detail23_2');
    }
    
    public function detail23_3() {
        $this->load->view('detail/detail23_3');
    }
    
    public function detail24() {
        $this->load->view('detail/detail24');
    }
    
    public function detail25() {
        $this->load->view('detail/detail25');
    }
    
    public function detail26() {
        $this->load->view('detail/detail26');
    }
    
    public function detail27() {
        $this->load->view('detail/detail27');
    }
    
    public function detail28() {
        $this->load->view('detail/detail28');
    }
    
    public function detail29() {
        $this->load->view('detail/detail29');
    }
    
    
    
    }